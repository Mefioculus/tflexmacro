...
public class Macro : MacroProvider {
    ...
    public void ОбновлениеКомплектаДокументов(Объекты объектыИзБП) {
        //ДиалогОжидания.Показать("Обновление комплекта документов", true);
        
        // Из объектов БП получаем технологические процессы
    ...
    private void ConvertGRBToPDF(TechnologicalSet setOfDocuments) {
        ...
        if (realFile != null) {
            ...
            if (folder != null) {
                Сообщение("Информация", folder.ToString());
                ...
                if (File.Exists(pathToRealFile)) {
                    // Если получилось сформировать файл подлинника, пытаемся разместить его в файловом справочнике
                    Сообщение("Информация", pathToRealFile);
                    ...
                    if (realFile != null) {
                        ...
                        Сообщение("Ошибка", "Не удалось произвести импорт файла в файловый справочник DOCs");
                    }
                    // Удаляем временный файл
                    // Можно отключать для того, чтобы проверять, происходило ли формирование подлинника
                    File.Delete(pathToRealFile);
                }
                else {
                    Сообщение("Ошибка", "При импорте в файловый справочник DOCs не был обнаружен файл подлинника на локальной машине");
                }
            }
            else {
