...
namespace TechnologicalMaps
{
    public class Macro : ReportMacroProvider
    {
        ...
        public void Карта_эскизов()
        {
            ...
            ВыполнитьМакрос(Макрос_Теги_17, "ОчиститьЗамечания");
            ВыполнитьМакрос(Макрос_Переменные_17, "ЗаполнитьОсновныеНадписи", операция);
            ...
            else
            {
                ...
                string operinfo = ВыполнитьМакрос(Макрос_Теги_17, "СводноеНаименование", операция);
                ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", "Операция '" + operinfo + "' не содержит эскиз.");

                //Создание пустого параграф-текста
                Строка страницаОтчёта = текст.Таблица.ДобавитьСтроку(Стр_шаблон);
            }

            ВыполнитьМакрос(Макрос_Теги_17, "ВывестиЗамечания");
            ВыполнитьМакрос(Макрос_Теги_17, "ОчиститьЗамечания");
        }
        //==========================================================================================================================//    
        ...
        public void Операционная_карта_с_эскизами()
        {
            ...
            ВыполнитьМакрос(Макрос_Теги_17, "ОчиститьЗамечания");
            ВыполнитьМакрос(Макрос_Переменные_17, "ЗаполнитьОсновныеНадписи", операция);
            ВыполнитьМакрос(Макрос_Переменные_17, "Переменные", операция);
            ...
            foreach (Переход переход in списокПереходов)
            {
                ...
                bool актуально = ВыполнитьМакрос(Макрос_Теги_17, "ЭлементАктуален", переход);
                if (!актуально)
                    continue;

                //Заполнение перехода
                if (!разбиватьТекстПерехода)
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуОиВремя", переход, текст, О_шаблон, букваО);
                else
                {
                    int ширина = ширинаДляГоризонтальнойФормыСВременем;
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуОиВремя", переход, текст, О_шаблон, ширина, букваО, true);
                }

                букваО = "";    //Следующий переход начнётся без символа "О" (если не будет оснащения или режимов)

                var оснащениеПерехода = переход.СвязанныеОбъекты["Оснащение ТП"];
                if (оснащениеПерехода.Any())   //если в перехода есть оснащение, то его нужно заполнить
                {
                    //Заполнение оснащения перехода
                    ВыполнитьМакрос(Макрос_Теги_17, "ВывестиОснащениеПерехода", переход, текст, ТП_шаблон, 0, 1, "; ");
                ...
                if (списокПараметровРежима.Length > 0)  //если в перехода есть режим, то его нужно заполнить
                {
                    ...
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуР_МехОбр", переход, строкаР, 1);
        ...
        public void Операционная_карта_контроля()
        {
            ...
            {
                string operinfo = ВыполнитьМакрос(Макрос_Теги_17, "СводноеНаименование", операция);
                ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", "Операция '" + operinfo
            ...
            string инструкцииДоОперации = ВыполнитьМакрос(Макрос_Теги_17, "ПолучитьИнструкции", операция, 1);
            string инструкцииПослеОперации = ВыполнитьМакрос(Макрос_Теги_17, "ПолучитьИнструкции", операция, 2);
            ...
            foreach (Переход переход in списокПереходов)
            {
                ...
                int kol = 0;
                foreach (var оснащениеТП in оснащениеПерехода)
                {
                    Объект оснащениеКаталога = оснащениеТП.СвязанныйОбъект["Оснащение из каталога оснащения"];
                    if (оснащениеКаталога != null)
                    {
                        обозначениеТО += оснащениеКаталога.Параметр["Обозначение"] + "\r\n";
                        наименованиеТО += оснащениеКаталога.Параметр["Наименование"] + "\r\n";
                    }
                    else
                        kol++;
                }
                ...
                if (kol > 0)
                {
                    string perehinfo = ВыполнитьМакрос(Макрос_Теги_17, "СводноеНаименование", переход);
                    ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", "В переходе '" + perehinfo
                                    + "' не указана связь между Оснащением перехода и Оснащением из справочника СТО для "
                                    + kol.ToString() + " из " + (оснащениеПерехода.Count).ToString() + " объекта(ов).");
                }
        ...
        public void Титульный_лист()
        {
            ...
            ВыполнитьМакрос(Макрос_Теги_17, "ОчиститьЗамечания");
            ВыполнитьМакрос(Макрос_Переменные_17, "ЗаполнитьОсновныеНадписи", техпроцесс);
        ...
        public void Ведомость_оснастки_или_оборудования(string ведомость, bool выводитьОперацию)
        {
            ...
            switch (ведомость)
            {
                ...
                case "ВОБ_ф2":
                    ...
                    break;
                default:
                    ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", "Неверно задан тип ведомости!");
                    break;
            }
            ...
            Операция[] списокОпераций = техпроцесс.Операции.Where(оп =>
            {
                ...
                bool актуально = ВыполнитьМакрос(Макрос_Теги_17, "ЭлементАктуален", оп);
                return актуально;
            }).ToArray();
            ...
            foreach (Операция операция in списокОпераций)
            {
                ...
                if (выводитьОперацию)
                {
                    ...
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуВ", операция, строкаВ);
                }

                //Заполнение оснащения операции
                if ((ведомость == "ВО_ф2") || (ведомость == "ВО_ф3"))
                    count += ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуТ_дляВО", операция, текст, Т_шаблон, выводитьОперацию, 1);

                //Заполнение оборудования операции
                if (ведомость == "ВОБ_ф2")
                    count += ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуД_дляВОБ", операция, текст, Д_шаблон);
        ...
        */
        ...
        public void Карта(string карта)
        {
            ...
            ВыполнитьМакрос(Макрос_Теги_17, "ОчиститьЗамечания");
            ВыполнитьМакрос(Макрос_Переменные_17, "ЗаполнитьОсновныеНадписи", техпроцесс);
            ВыполнитьМакрос(Макрос_Переменные_17, "Переменные", техпроцесс);
            ...
            switch (карта)
            {
                ...
                case "МК_ф3":
                    ...
                    break;
                default:
                    ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", "Неверно задан тип технологической карты!");
                    break;
            }
            ...
            if (карта != "МОК_ф2")
            {
                ...
                for (int i = 1; i < материалы.Count; i++)
                {
                    //Заполняем строку материала
                    Строка строкаМТ = текст.Таблица.ДобавитьСтроку(МТ_шаблон);
                    строкаМТ["букваМ"].Текст = букваМ;
                    строкаМТ["МатериалТП"].Текст = ВыполнитьМакрос(Макрос_Теги_17, "СформироватьСтрокуМатериалаТП_дляМК", материалы[i]);
            ...
            if (сборочная && !списокОпераций.Any(оп => оп.Тип.ПорожденОт("Сборочная операция")))
            {
                ...
                ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания",
            ...
            {
                //Выводить ли операцию в карту
                актуально = ВыполнитьМакрос(Макрос_Теги_17, "ЭлементАктуален", операция);
                if (!актуально)
                    continue;

                //Наличие видов инструкции в операции
                Tuple<bool, bool, bool> наличиеИнструкций = ВыполнитьМакрос(Макрос_Теги_17, "НаличиеИнструкций", операция);
                ...
                if (карта != "МОК_ф2")
                {
                    ...
                    if (flagIDO)
                    {
                        ...
                        ВыполнитьМакрос(Макрос_Теги_17, "ВывестиИнструкции", операция, строкаИДО, 1);
                    }
                }
                ...
                if (карта == "КТП" || карта == "МОК_ф1" || карта == "МОК_ф2" || карта == "МК_ф1")
                {
                    ...
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуА", операция, строкаА);

                    //Заполнение оборудования и исполнителей
                    Строка строкаБ = текст.Таблица.ДобавитьСтроку(Б_шаблон);
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуБ_дляМК", операция, строкаБ);
                }
                ...
                if (карта == "МОК_ф3" || карта == "МК_ф3")
                {
                    ...
                    if (flagIPO)
                    {
                        ...
                        ВыполнитьМакрос(Макрос_Теги_17, "ВывестиИнструкции", операция, строкаГ, 2);
                    }

                    //Проверка наличия оборудования
                    var оборудованиеОперации = операция.СвязанныеОбъекты["Оборудование"];
                    bool flagOb = false;
                    foreach (var оборудование in оборудованиеОперации)
                    {
                        if (оборудование.Тип.ПорожденОт("Оборудование"))
                            flagOb = true;
                    }
                    ...
                    if (flagOb) //если оборудование есть
                    {
                        ...
                        ВыполнитьМакрос(Макрос_Теги_17, "ВывестиОборудование", операция, строкаД);
                    }

                    //Заполнение исполнителей
                    Строка строкаЕ = текст.Таблица.ДобавитьСтроку(Е_шаблон);
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуЕ", операция, строкаЕ);
                }
                ...
                if (карта != "МОК_ф2")
                {
                    ...
                    {
                        var списокМатериалов = операция.СвязанныеОбъекты["Материалы операции"];
                        foreach (var материал in списокМатериалов)
                        {
                            Строка строкаМ = текст.Таблица.ДобавитьСтроку(М_шаблон);
                            ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуМ_дляМК", материал, строкаМ);
                        }
                    }
                }

                //Заполнение оснащения операции
                ВыполнитьМакрос(Макрос_Теги_17, "ВывестиОснащениеОперации", операция, текст, ТО_шаблон, 0, 1, 1, "; ");

                //Заполнение комплектующих на операцию
                if (сборочная)
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуКЛ", операция, текст, КЛ_шаблон, горизонтальнаяФорма);
                ...
                if (карта == "КТП" || карта == "МОК_ф1" || карта == "МОК_ф2" || карта == "МОК_ф3")
                {
                    ...
                    {
                        //Выводить ли переход в карту
                        актуально = ВыполнитьМакрос(Макрос_Теги_17, "ЭлементАктуален", переход);
                        if (!актуально)
                            continue;

                        //Заполнение перехода
                        if (!разбиватьТекстПерехода)
                            ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуО", переход, текст, О_шаблон, букваО);
                        else
                        {
                            int ширина = горизонтальнаяФорма ? ширинаДляГоризонтальнойФормы : ширинаДляВертикальнойФормы;
                            ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуО", переход, текст, О_шаблон, ширина, букваО, true);
                        }
                        ...
                        if (карта == "КТП" || карта == "МОК_ф2")
                        {
                            ...
                            if (списокПараметровРежима.Length > 0)  //если в перехода есть режим, то его нужно заполнить
                            {
                                ...
                                if (карта == "КТП")
                                    ...
                                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуР_МехОбр", переход, строкаР, 1, true);
                ...
                if (карта != "МОК_ф2")
                {
                    ...
                    if (flagIPP)
                    {
                        ...
                        ВыполнитьМакрос(Макрос_Теги_17, "ВывестиИнструкции", операция, строкаИПП, 3);
                    }
                }
        ...
        public void Комплектовочная_карта(bool горизонтальная)
        {
            ...
            foreach (Операция операция in списокОпераций)
            {
                ...
                if (!горизонтальная) //форма 6
                {
                    ...
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуВ", операция, строкаКЛ);
                    строкаКЛ["букваВ"].Текст = "В";
                }
                else                 //форма 7
                {
                    строкаКЛ = текст.Таблица.ДобавитьСтроку(КЛ_шаблон);
                    ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуВ_безНаименования", операция, строкаКЛ);
                }

                ReferenceObject operation = (ReferenceObject)операция;
                var linkAssemblyOperationParts = operation.Links.ToManyToComplexHierarchy[new Guid("25a393dc-8f97-4e25-aa68-30f8382cd756")];            // Связь Комплектующие
                if (linkAssemblyOperationParts.Objects.GetCount() < 1)
                {
                    ВыполнитьМакрос(Макрос_Теги_17, "ДобавитьЗамечания", String.Format("В операции '{0}' не заданы компоненты.", ВыполнитьМакрос(Макрос_Теги_17, "СводноеНаименование", операция)));
                    continue;
                }
                List<Объект> компоненты = linkAssemblyOperationParts.Objects.Select(t => Объект.CreateInstance(t, Context)).ToList();
                var нематериалы = компоненты.Where(к => !к.Тип.ПорожденОт("Материал")).ToList();
                var материалы = компоненты.Where(к => к.Тип.ПорожденОт("Материал")).ToList();

                //Заполнение строк отчёта по деталям и сборочным единицам
                int i = 0;
                foreach (var link in linkAssemblyOperationParts)
                {
                    var childObject = link.ChildObject;
                    Объект компонент = Объект.CreateInstance(childObject, Context);
                    if (!компонент.Тип.ПорожденОт("Материал"))
                    {
                        ...
                        try
                        {
                            строкаКЛ["Поз"].Текст = link[new Guid("ab34ef56-6c68-4e23-a532-dead399b2f2e")].Value.ToString();                  // Позиция
                        }
                        catch
                        {}
                        
                        double количество = (double)link[new Guid("3f5fc6c8-d1bf-4c3d-b7ff-f3e636603818")].Value;                         // Количество
                        try
                        {
                            string ЕИ = link[new Guid("444377c3-f73b-439a-b997-9b20c5707231")].Value.ToString();                              // Единица измерения
                            ВыполнитьМакрос(Макрос_Теги_17, "ЗаполнитьСтрокуКомпонента", строкаКЛ, компонент, количество, ЕИ);                        
                        }
                        catch
                        {}
        ...
        private string ПолучитьВсеИнструкцииПоОперациям(ТехнологическийПроцесс техпроцесс)
        {
            ...
            foreach (Операция операция in техпроцесс.Операции)
            {
                ...
                {
                    if (ВыполнитьМакрос(Макрос_Теги_17, "инструкцияОТ", инструкцияОперации))
                    {
                        if (инструкцияОперации.Шифр != String.Empty && !инструкции.Contains(инструкцияОперации.Шифр)) // указан шифр инструкции и его нет в списке
                        {
                            ...
                            var исполнительОперации = операция.СвязанныеОбъекты["Исполнители операции"].FirstOrDefault(); // первый исполнитель в списке
                            string профессия = исполнительОперации != null && исполнительОперации.СвязанныйОбъект["Профессия"] != null ? исполнительОперации.СвязанныйОбъект["Профессия"]["Код профессии"].ToString() : String.Empty;
