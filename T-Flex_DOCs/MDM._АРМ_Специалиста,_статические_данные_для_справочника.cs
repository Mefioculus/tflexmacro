...
public class AsuNsiARMStatic : MacroProvider
{
    ...
    public void ПоказатьДиалогВыбораСправочника()
    {
        if (Вопрос("Хотите запустить в режиме отладки?"))
        {
            System.Diagnostics.Debugger.Launch();
        ...
        var диалог = СоздатьДиалогВыбораОбъектов(ИсточникиДанных);
        диалог.МножественныйВыбор = false;
        if (!диалог.Показать())
            return;

        var выбранныйОбъект = диалог.ФокусированныйОбъект;
        if (выбранныйОбъект == null)
            return;

        Guid справочникGuid = выбранныйОбъект[ИсточникиДанныхПараметрАналог];
        if (справочникGuid == Guid.Empty)
            return;

        NameReference = справочникGuid;
