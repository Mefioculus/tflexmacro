    ...
    public class Macro : MacroProvider
    {
        ...
        private static class Guids
        {
            ...
            public static class Links
            {
                ...
                public static readonly Guid ДокументыФайлы = new Guid("9eda1479-c00d-4d28-bd8e-0d592c873303");
            }
        ...
        public void СоздатьИзменение(Объект номенклатура, Объект документ)
        {
            if (!Question("Создать изменение?"))
                return;

            CreateChangingForNomenclatureObject((ReferenceObject)номенклатура as NomenclatureObject);
            документ.Подключить("d8d1f02d-5cfe-452d-8ac9-c4f4d33a57de", изменение);                                              // Изменения
            документ.Сохранить();
        }
        ...
        private void CreateChangingForNomenclatureObject(NomenclatureObject nomenclatureObject)
        {
            ...
            string revision = nomenclatureObject.SystemFields.RevisionName;
            ...
            var currentHierarchyLink = (ComplexHierarchyLink)ТекущееПодключение;
            var parent = currentHierarchyLink != null ? currentHierarchyLink.ParentObject : null;
            var minorLevel = nomenclatureObject.Class.RevisionNamingRule.RevisionLevels[1];
            NomenclatureObject newMinorRevision = nomenclatureObject.CreateRevision(minorLevel, parent) as NomenclatureObject;

            // Создаем пустую применямость
            // ????????????????????????????????????????? пока выключили  ?????????????????????????????????????????
            
            /*var referenceInfoApplicabilityInProducts = Context.Connection.ReferenceCatalog.Find(new Guid("c9a48b8e-8af4-483e-8c92-f0f39f3df65f"));
            var referenceApplicabilityInProducts = referenceInfoApplicabilityInProducts.CreateReference();
            var applicabilityInProductsReferenceObject = referenceApplicabilityInProducts.CreateReferenceObject();
            applicabilityInProductsReferenceObject.EndChanges();

            var complexLink = newMinorRevision.GetParentLink(parent);

            complexLink.BeginChanges();
            complexLink.AddLinkedObject(new Guid("fff32559-ef02-4e31-88fa-da36c23985da"), applicabilityInProductsReferenceObject);
            complexLink.EndChanges();
            // ????????????????????????????????????????? пока выключили  ?????????????????????????????????????????
            */
           
           
            // Создаем статус "Разрабатывается"
            ReferenceObject typeStatusDesigning = (ReferenceObject)НайтиОбъект("Типы статусов", "Наименование", "Разрабатывается");
            ...
            statusReferenceObject[new Guid("b6c6f4cb-2548-4224-b8ed-f3abf82c3e8d")].Value = "Разрабатывается";
            ...
            newMinorRevision.BeginChanges();
            newMinorRevision.AddLinkedObject(new Guid("13903cba-7b7d-464a-8481-56ef6a3f36dc"), statusReferenceObject);
            newMinorRevision.EndChanges();
            ...
            */
            ...
            changing.SetLinkedObject(Guids.Links.ИзмененияЦелевойВариант, newMinorRevision);
            if (nomenclatureObject != null)
                changing.SetLinkedObject(Guids.Links.ИзмененияИсходныйВариант, nomenclatureObject);
            if (nomenclatureObject != null)
                changing.SetLinkedObject(Guids.Links.ИзмененияАктуальныйВариант, nomenclatureObject);
            ...
            CopyFilesForChanging(nomenclatureObject.LinkedObject, changing, folderForChangingFiles, newMinorRevision.LinkedObject);

            changing.EndChanges();
            изменение = Объект.CreateInstance(changing, Context);
        }

        private void CopyFilesForChanging(ReferenceObject linkedReferenceObject, ReferenceObject changing, TFlex.DOCs.Model.References.Files.FolderObject folderForChangingFiles, ReferenceObject newMinorRevision = null)
        {
            ...
            var newMinorRevisionDocument = newMinorRevision as EngineeringDocumentObject;
            newMinorRevisionDocument.BeginChanges();
            ...
            if (checkInFiles.Any())
            {
                try
                {
                    ...
                    foreach (FileObject file in checkInFiles)
                    {
                        ...
                        newMinorRevisionDocument.AddLinkedObject(Guids.Links.ДокументыФайлы, file);
                    }
            ...
            newMinorRevisionDocument.EndChanges();
        }
        ...
        public void ПринятьНаХранение()
        {
            ...
            //System.Diagnostics.Debugger.Launch();
            //System.Diagnostics.Debugger.Break();
            foreach (var fileGRB in documentFiles.Where(t => t.Class.IsInherit(new Guid("73c01001-41d6-454d-816f-311528e5115d"))))	// порождены от grb
            {
                int fileId = ВыполнитьМакрос("PDM-Экспорт файла в tif_Новый", "RunFromObject", fileGRB);
                Объект file = НайтиОбъект("Файлы", "[Id] = '" + fileId + "'");
                if (file != null)
                {
                    FileObject f = (ReferenceObject)file as FileObject;
                    files.Add(f);
                }
            }

            documentFiles.AddRange(files);
        ...
        private bool DoActionWithDocumentFiles(ReferenceObject document, List<FileObject> documentFiles, ref List<ReferenceObject> storageObjects)
        {
            ...
            var approvedFiles = documentFiles.ToList();//.Where(file => IsReferenceObjectInStage(file, "Утверждено")).ToList();
