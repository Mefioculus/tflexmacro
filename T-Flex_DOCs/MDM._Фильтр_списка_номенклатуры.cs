...
public class MacroMDMFilterNomen : MacroProvider
{
    public MacroMDMFilterNomen(MacroContext context) : base(context) { }
    ...
    private static List<int> objectsID = new List<int>();

    public override void Run()
    {
    }

    public void ЗаполнитьФильтрПоТекущемуОбъекту()
    {
        ...
        var диалог = СоздатьДиалогВвода("Введите обозначение");
        диалог.ДобавитьСтроковое("Обозначение", обязательное: true);
        if (!диалог.Показать())
            return;

        var списокНоменклатурыReference = Context.Connection.ReferenceCatalog.Find(СписокНоменклатуры_Guid).CreateReference();
        List<ReferenceObject> списокНоменклатуры;
        using (var filter = new Filter(списокНоменклатурыReference.ParameterGroup))
        {
            ...
            списокНоменклатуры = списокНоменклатурыReference.Find(filter);
        }
        ...
        objectsID = allObjects.Select(ob => ob.SystemFields.Id).ToList();
    }

    public List<int> ПолучитьОбъекты()
    {
        return objectsID;
    }
}
