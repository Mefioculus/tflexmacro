...
public class Macro70 : MacroProvider
{
    ...
    Guid правило_обмена_Guid = new Guid("7a306e04-bc6c-4d89-9567-d5248db5b536");
    ...
    public void Import()
    {
        ...
        switch (masterDataBinding.ExternalDataPlatform)
        {
            case ExternalDataPlatform.Xml:
                {
                    ...
                    if (!ApplicationManager.OpenDialog(selectViewModel, null))
                        return;
                }
                break;
            ...
            case ExternalDataPlatform.Conversion:
                {
                    if (!RunExportFileViewModel.OpenAsync(settings, masterDataBinding.Dispatcher, null, CancellationToken.None).Result)
                        return ;
                }
                break;
            default:
    ...
    protected DataExchangeResults Run(MasterDataBindingReferenceObject masterDataBinding, DataExchangeSettings settings)
    {
        ...
        {
            ApplicationManager.ShowMessage(validationResult.GetErrorMessage());
            return null;
        }
