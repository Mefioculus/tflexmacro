...
{
    public class MacroAEMChanges : ProcessActionMacroProvider
    {
        /// <summary>
        ...
        public MacroAEMChanges(EventContext context)
            : base(context)
        {
        }
        ...
        {
            ПоказатьДиалогСвойств(ТекущийОбъект.СвязанныйОбъект["[Связанный объект].[ac46ca13-6649-4bbb-87d5-e7d570783f26]->[d708e1b4-2a1a-499c-aaaf-be5828e6377e]"]);
        }
        ...
        public void ПринятьНаХранение(NomenclatureReferenceObject nomenReferenceObj)
        {
            var nomenclatureObject = nomenReferenceObj as NomenclatureObject;
            if (nomenclatureObject == null)
                Error(String.Format("Текущий объект '{0}' не является номенклатурным объектом.", Context.ReferenceObject));
        ...
        private StringBuilder _logError = new StringBuilder();

        public void ПринятьНаХранение_1()
        {
            //Получаем все объекты
            List<ReferenceObject> allObjs = GetAllBPObject();

            foreach (var referenceObject in allObjs)
            {
                EngineeringDocumentObject document;
                NomenclatureObject nomenclatureObject;

                if (!GetDocument(referenceObject,out nomenclatureObject, out document))
                    continue;
                ...
                if (inventoryCard == null)
                {
                    _logError.Append(String.Format("Ошибка создания инвентарной карточки для документа '{0}'.", document));
                    continue;
                }
...
/*              //Показываем диалог инвентарной карточки
                if (!ShowPropertyDialog(RefObj.CreateInstance(inventoryCard, Context)))
                {
                    continue;
                }
*/
            ...
            if (!string.IsNullOrEmpty(_logError.ToString()))
                Сообщение("Предупреждение",_logError.ToString());
        ...
        private List<ReferenceObject> GetAllBPObject()
        {
            var allObjs = new List<ReferenceObject>();
            //Добавляем объекты
            var currentObjects = Объекты.Select(obj => (ReferenceObject)obj);

            foreach (var currentObject in currentObjects)
            {
                //Добавляем текущий объект
                allObjs.Add(currentObject);

                //Получаем дочерние объекты
                var children = currentObject.Children.ToList();

                //Проверяем что дочерние есть
                if (children.Any())
                    allObjs.AddRange(children);
            }

            return allObjs;
        }

        private bool GetDocument(ReferenceObject referenceObject, out NomenclatureObject nomenclatureObject, out EngineeringDocumentObject document)
        {
            document = null;

            nomenclatureObject = referenceObject as NomenclatureObject;
            if (nomenclatureObject == null)
            {
                _logError.Append(String.Format("Текущий объект '{0}' не является номенклатурным объектом.",
                        Context.ReferenceObject));
                return false;
            }

            document = nomenclatureObject.LinkedObject as EngineeringDocumentObject;
            if (document == null)
            {
                _logError.Append(String.Format("У номенклатурного объекта '{0}' отсутствует связанный документ.",
                        nomenclatureObject));
                return false;
            }
        ...
        public void ПринятьНаХранение_2()
        {
            var objs = GetAllBPObject();

            foreach (var referenceObject in objs)
            {
                EngineeringDocumentObject document;
                NomenclatureObject nomenclatureObject;
        ...
        public void ПринятьНаХранение_3()
        {
            Объекты дочки = Переменные["Дочерние"];
            foreach (var объектБП in Объекты)
                дочки.Add(объектБП);
            var objs = дочки.Select(obj => (ReferenceObject)obj);
            ...
            foreach (var referenceObject in objs)
            {
                ...
                // все объекты, у которых меняем стадию на хранение
                List<ReferenceObject> storageObjects = new List<ReferenceObject> { nomenclatureObject };
                List<FileObject> documentFiles = new List<FileObject>();
                var resIDs = new System.Collections.Generic.List<int>();
                var files = document.GetObjects(new Guid("9eda1479-c00d-4d28-bd8e-0d592c873303")); //Связь Документы - Файлы
                
                foreach (var файл in files)
                {
                    if (файл.SystemFields.Stage.ToString() != "Хранение" && !resIDs.Contains(файл.Id))
                    {
                        documentFiles.Add(файл as FileObject);
                        resIDs.Add(файл.Id);
                    }
                }


                //var documentFiles = файлы.Select(obj => (ReferenceObject)obj as FileObject).ToList();
        ...
        {
            if (document[new Guid("9781595e-7c37-42cb-8033-a9e192e7106b")].Value.ToString() == "БЧ")
                return true;
        ...
        private void FillInventoryCard(ReferenceObject inventoryCard, bool fillOnButton = false)
        {
            ...
            if (document == null)
            {
                _logError.Append("Внимание! У инвентарной карточки отсутствует связанный документ.");
