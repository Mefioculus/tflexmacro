...
namespace PDM_ChangeNotification
{
    ...
    public class Macro : ProcessActionMacroProvider
    {
        ...
        private void CopyHierarchyLinkParameters(NomenclatureObject targetNomenclature, NomenclatureObject actualNomenclature)
        {
            ...
            var detachableChildren = new List<NomenclatureObject>();

            foreach (var actualChild in childrenOfActualNomenclature)
            {
                var targetChild = childrenOfTargetNomenclature.FirstOrDefault(tChild => tChild.SystemFields.Guid == actualChild.SystemFields.Guid);
                if (targetChild is null)
                {
                    var link = actualNomenclature.GetChildLink(actualChild);
                    actualNomenclature.DeleteLink(link);
                    actualNomenclature.DeleteLink(link);
                }
                else
                {
                    var targetLink = targetNomenclature.GetChildLink(targetChild) as NomenclatureHierarchyLink;
                    if (!targetLink.EndDate.IsNull)
                    {
                        var link = actualNomenclature.GetChildLink(actualChild);
                        actualNomenclature.DeleteLink(link);
                    }
            ...
            var newAttachedChildren = childrenOfTargetNomenclature
                .Where(tn => !childrenOfActualNomenclature.Any(an => an.SystemFields.Guid == tn.SystemFields.Guid)).ToList();

            foreach (var ac in newAttachedChildren)
                actualNomenclature.CreateChildLink(ac).EndChanges();

            foreach (var tnChild in childrenOfTargetNomenclature)
            {
                var correspondingANChild = childrenOfActualNomenclature
                    .FirstOrDefault(aChild => aChild.SystemFields.Guid == tnChild.SystemFields.Guid);

                if (correspondingANChild is null)
                    continue;

                var tnComplexHierarchyLink = targetNomenclature.GetChildLink(tnChild);
                if (tnComplexHierarchyLink is null)
                    continue;

                var anComplexHierarchyLink = actualNomenclature.GetChildLink(correspondingANChild);
                if (anComplexHierarchyLink is null)
                    continue;

                anComplexHierarchyLink.BeginChanges();

                foreach (var param in tnComplexHierarchyLink.Where(par => !par.ParameterInfo.IsSystem && !par.IsReadOnly))
        ...
        private void ProcessInventoryCard(FileObject fileInDocument, ReferenceObject document)
        {
            ...
            RunMacro(InventoryCardMacroGuid, "ProcessInventoryCard", inventoryCard, document, fileInDocument, true, false);
        }
