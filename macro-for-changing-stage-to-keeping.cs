...
namespace TechnologicalPDM_Changing
{
    public class Macro : MacroProvider
    {
        ...
        public void ПринятьНаХранениеТП()
        {
        	...
        	ПринятьНаХранениеТПОбщий(currentTP);
        }
        
        public void ПринятьНаХранениеТПИзПерехода(Объекты objectsFromBP)
        {
        	List<ReferenceObject> listOfTP = new List<ReferenceObject>();
        	
        	foreach (Объект obj in objectsFromBP)
        	{
        		var document = (ReferenceObject)obj;
        		if (document.Class.IsInherit(Guids.Classes.ТехнологическийПроцесс))
        			listOfTP.Add(document);
        		
        		if (listOfTP.Count == 0)
        		{
        			Сообщение("Инфомрация", "В бизнесс-процессе не было найдено ни одного технологического процесса для передачи на хранение");
        			return;
        		}
        		
        		foreach (ReferenceObject tp in listOfTP)
        			ПринятьНаХранениеТПОбщий(tp);
        	}
        }
        
        
        private void ПринятьНаХранениеТПОбщий(ReferenceObject currentTP)
        {
