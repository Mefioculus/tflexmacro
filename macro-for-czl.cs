...
public class Macro : MacroProvider
{
    ...
    #region созданиеОбъектовАрхиваИзСканов
/*

    // Метод для создания объектов на основании документов в папке для сканирования
    public void созданиеОбъектовАрхиваИзСканов()
    {
        // Метод предназначен для механизма создания объектов на основании файлов сканов протоколов, 
        // для того, чтобы создание:w
        // можно было производить пакетным способом

        // Для начала от пользователя нужно получить расположение папки, которую макрос будет просматривать
        // (и в которой должны быть расположены pdf)
        // Переменная для хранения пути
        GetDirectoryWithScans();
        // Завершение работы макроса в том случае, если не был указан путь к директории со сканами
        if (directoryWithPDF == null) return;

        // Далее мы проверяем полученную директорию на наличие файлов, которые соответствуют заданной маске
        GetFiles();

        // Производим проверку, были ли найдены файлы
        if (listOfFilesOnProcessing == null)
        {
            Сообщение("Информация", "В папке назначения не было найдено ни одного файла, удовлетворяющего условиям поиска");
            return;
        }

        // Производим подготовительную работу для того, чтобы можно было добавить сканы в папку файлового справочника
        // Для добавления сканов в папку для архива ЦЗЛ в файловом справочнике, создаем экземпляр файлового справочника
        FileReference fileReference = new FileReference(Context.Connection);
        // Задаем целевую папку для импорта файлов
        FolderObject directory = (FolderObject)fileReference.FindByRelativePath("Архив ЦЗЛ");
        // Получаем тип файла, который потребуется для создания нового файла в справочнике Файлы
        FileType type = fileReference.Classes.Find(Guids.Types.ТипPDF);

        AddFileInArchive(ref directory, ref type);

        Сообщение("Информация", "Работа макроса завершена");
    }

    
    #region Служебные методы для создания объектов из сканов

    private void GetDirectoryWithScans()
    {
        if (Вопрос(string.Format("Использовать папку для сканирования по умолчанию?\n{0}", ГлобальныйПараметр["Папка для сканирования (ЦЗЛ)"])) == true)
        {
            GetDefaultPathToDirectory();
        }
        else
        {
            GetDirectoryFromBrowse();
        }
    }
    
    private void GetDefaultPathToDirectory()
    {
    	// Метод для получения стандартного расположения папки для сканирования
    	// В данный момент макрос всего лишь возвращает заложенную в нем строку,
    	// но в дальнейшем он будет обращаться к глобальному параметру для получения
    	// пути
        string path = ГлобальныйПараметр["Папка для сканирования (ЦЗЛ)"];
        // Проверка пути на существование
        if (!Directory.Exists(path))
        {
            Сообщение("Информация", "Директория, указанная по умолчанию не существует. Выберите директорию самостоятельно");
            GetDirectoryFromBrowse();
        }
        else
        {
            directoryWithPDF = path;
        }
    }

    private void GetDirectoryFromBrowse()
    {
        ...
        {    	                     
    	   Forms.FolderBrowserDialog dialog = new Forms.FolderBrowserDialog();
            ...
            // Проверка пути, который был открыт в последний запуск макроса на доступность
            if (Directory.Exists(ГлобальныйПараметр["Последняя выбранная папка"]))
            {
                dialog.SelectedPath = ГлобальныйПараметр["Последняя выбранная папка"];
            }
        
            if (dialog.ShowDialog() == Forms.DialogResult.OK)
            {
                Forms.MessageBox.Show(dialog.SelectedPath);
                directoryWithPDF = dialog.SelectedPath;

                // Сохраняем SelectedPath для того, чтобы в дальнейшем обзор у пользователя открывался с ранее выбранной папки
                ГлобальныйПараметр["Последняя выбранная папка"] = directoryWithPDF;
            }
        });
    }
    ...
    {
        // Метод для получения перечня файлов, выбранных по маске
        listOfFilesOnProcessing = Directory.GetFiles(directoryWithPDF, "Протокол № ????_??.??.??.pdf");

        if (listOfFilesOnProcessing.Length > 0)
        {
            // Выводим сообщение на экран о найденный файлах
            string message = "Список обнаруженных в директории файлов, соотвутствующих заложенному правилу названия файлов:\n";
            foreach(string path in listOfFilesOnProcessing)
            {
        ...
        {
            listOfFilesOnProcessing = null;
            return;
        }

        // Далее, производим проверку и фильтрацию
        // Дла начала получаем данные, которые содержатся в справочнике DOCs
        Объекты архивЦЗЛ = НайтиОбъекты(Guids.References.АрхивЦЗЛ.ToString(), "[Тип] = 'Протокол входного контроля'");
        // Получаем все номера объектов в Архиве ЦЗЛ
        List<int> numberItem = new List<int>();
        foreach(Объект протокол in архивЦЗЛ)
        {
            // Получаем все номера
            numberItem.Add(протокол.Параметр["№ Свидетельства"]);

        }

        if (numberItem.Count == 0)
        {
            // Не фильтруем данные на основе тех позиций, которые имеются в справочнике,
            // так как в справочнике не имеется позиций
            return;
        }

        // Производим фильтрацию объектов
        listOfFilesOnProcessing = (from file in listOfFilesOnProcessing
                                    where !numberItem.Contains(int.Parse(Path.GetFileName(file).Split('_')[0].Split(' ')[2]))
                                    select file).ToArray();
        return;
    }

    private void AddFileInArchive(ref FolderObject directory, ref FileType type)
    {
        if (listOfFilesOnProcessing.Length == 0)
        {
            Сообщение("Информация", "В директории для сканирования не было обнаружено файлов, которые можно добавить в базу данных");
            return;
        }
        // Выводим поврос для пользователя с списком файлов, которые будут обрабатываться и спрашиваем у него,
        // хочет ли он продолжить
        string message = "Создать следующие записи в Архив ЦЗЛ?\n";
        foreach(string path in listOfFilesOnProcessing)
        {
            message += string.Format("{0}\n", Path.GetFileName(path));
        }

        if (Вопрос(message) != true)
        {
            return;
        }
        // Приступаем к созданию файлов в справочнике
        foreach (string file in listOfFilesOnProcessing)
        {
            // Получаем значение номера записи и даты
            string fileName = Path.GetFileNameWithoutExtension(file);

            // Получаем значение даты из название файла для передачи в объект
            int year = int.Parse(fileName.Split('_')[1].Split('.')[2]);
            year += 2000;
            int month = int.Parse(fileName.Split('_')[1].Split('.')[1]);
            int day = int.Parse(fileName.Split('_')[1].Split('.')[0]);

            DateTime date = new DateTime(year, month, day);

            // Получаем номер протокола
            int numberProtocol = int.Parse(fileName.Split('_')[0].Replace("Протокол № ", ""));

            // Далее, создаем новый объект в справочнике
            Объект новыйПротокол = СоздатьОбъект(Guids.References.АрхивЦЗЛ.ToString(), "Протокол входного контроля");
            новыйПротокол.Параметр["№ Свидетельства"] = numberProtocol;
            новыйПротокол.Параметр["Дата"] = date;

            // Код, который добавляет сканы в папку, и подключает файлы по связи
            // Папка для файлов уже была указана в начале макроса, здесь мы ее уже испльзуем
            FileObject newFile = directory.CreateFile(file, fileName, string.Format("{0}.pdf", fileName) , type, 
                    new ImportParameters()
                    {
                        // Указываем параметр для AutoCheckIn
                        AutoCheckIn = true,
                        // Указываем параметр, что не требуется обновлять существующие файлы
                        UpdateExistingFiles = false,
                    }
            );

            // newFile.CheckIn(string.Format("Добавление в файловый справочник файла '{0}.pdf'", fileName));
            Desktop.CheckIn((DesktopObject)newFile,
                            string.Format(string.Format("Добавление в файловый справочник файла '{0}.pdf'", fileName)),
                            false);
            
            // Далее, подключаем объект по связи
            новыйПротокол.Подключить("Сканированные документы", НайтиОбъект("Файлы", "Наименование", string.Format("{0}.pdf", fileName)));
            
            новыйПротокол.Сохранить();
        }
    }

    #endregion Служебные методы для создания объектов из сканов
    */
    #endregion созданиеОбъектовАрхиваИзСканов
    ...
    public string ПолучитьДанныеДляОтчета() {
        ...
        switch ((int)record[Guids.Props.ПараметрВидТаблицы].Value) {
            ...
            case таблицаПоОбразцам:
                ...
                resultDataClass.Add("Вид проката");
                resultDataClass.Add("Сводный размер");
                resultDataClass.Add("Предел прочности");
                resultDataClass.Add("Предел текучести");
                resultDataClass.Add("Относительное удлинение");
                resultDataClass.Add("Прочность при изгибе");
